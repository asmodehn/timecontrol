...
class TestCommandRun(unittest.TestCase):
    ...
    def test_result_from_integers(self, result: int):
        sig = inspect.signature(lambda: result)
        ba = sig.bind()

        ok = CommandRun(call=CommandCalled(bound_arguments=ba), result=Result.Ok(result))
        err = CommandRun(call=CommandCalled(bound_arguments=ba), result=Result.Err(result))
...
class TestCommandLog(unittest.TestCase):
    ...
    def test_logger_mapping(self):
        ...
        f = CommandLog()
        sig = inspect.signature(lambda: 63)
        ba = sig.bind()

        # note we override the result in the event !
        e1 = CommandRun(timestamp=self.clock, call=CommandCalled(timestamp=self.clock, bound_arguments=ba), result=Result.Ok(42))
        assert f(e1) == e1  # identity from caller point of view

        assert f[self.clock] == {e1}

        e2 = CommandRun(timestamp=self.clock, call=CommandCalled(timestamp=self.clock, bound_arguments=ba), result=Result.Ok(42))
        assert f(e2) == e2
        ...
        e3 = CommandRun(timestamp=self.clock, call=CommandCalled(bound_arguments=ba), result=Result.Ok(42))
        assert f(e3) == e3
